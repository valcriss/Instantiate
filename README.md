...
Instantiate can deploy to a Swarm cluster by rendering your compose file and
running `docker stack deploy`. Set `orchestrator` to `swarm` in your
`config.yml` and provide a standard Compose template:
...
For Kubernetes deployments, set the orchestrator to `kubernetes` and place your
manifests in `.instantiate/docker-compose.yml`. Instantiate applies the rendered
file with `kubectl`:
